...
def generate_detections(encoder, mot_dir, detection_dir='./MOT15/train/TUD-Campus/det/det.txt'):
    ...
    for sequence in os.listdir(mot_dir):
        ...
        for frame_idx in range(min_frame_idx, max_frame_idx + 1):
            ...
            trans=np.array(detections_out)
            #print(trans)
            
            seq_info = gather_sequence_info("./MOT15/train/TUD-Campus", "./MOT15/train/TUD-Campus/det/det.txt",frame_idx)
            results = []
      

            detections = create_detections(
            seq_info["detections"], frame_idx, 0)
            ...
            via=visualization.Visualization(seq_info, 5)
            via.set_image(image.copy())
            via.draw_trackers(tracker.tracks)
            
            via.run2(frame_callback2)
            ...
            f = open("/home/a/Git/deep_sort/result/hypotheses.txt", 'a')
            for row in results:
...
########################################################################################
def gather_sequence_info(sequence_dir, detection_file,frame_idx):
    ...
    image_filenames = {
        int(os.path.splitext(f)[0]): os.path.join(image_dir, f)
        for f in os.listdir(image_dir)}
    #image_filenames2={:os.path.join(image_dir, f) for f in os.listdir(image_dir)}
    #print(image_filenames)
    ...
    #print(seq_info["image_size"])  #480.640
    #print(seq_info["sequence_name"]) #TUD-CAMPUS
    return seq_info
...
def create_detections(detection_mat, frame_idx, min_height=0):
    ...
    """
    print("create_detection function")
...
def run(sequence_dir, detection_file, output_file, min_confidence,
    ...
    #seq_info = gather_sequence_info(sequence_dir, detection_file,frame_idx)
    #seq_info["image_filenames"].clear()
    #seq_info["image_filenames"]={int(frame_idx):'./MOT15/train/TUD-Campus/img1/%06d.jpg'%frame_idx}
    #metric = nn_matching.NearestNeighborDistanceMetric(
        #"cosine", max_cosine_distance, nn_budget)
    #tracker = Tracker(metric)
    #results = []
    #global a
    def frame_callback(via, frame_idx):
        ...
        #via.draw_groundtruth(track.track_id)
        via.draw_trackers(tracker.tracks)
        
        #plt.savefig('pic/TUD-Campus/%06d.jpg'%frame_idx)
        #cv2.imshow('test',image)
        #cv2.imwrite('pic/TUD-Campus/%06d.jpg',image)
        print('test')
...
def frame_callback2(vis, frame_idx):
        print("Processing frame %05d" % frame_idx)
...
if __name__ == "__main__":
    ...
    generate_detections(f, args.mot_dir, args.detection_dir)
